    ...
    <x-slot name="header">
        <div class="flex items-center">
            ...
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('Editar Horario de Publicación') }}
            </h2>
        </div>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form action="{{ route('calendar.update', $schedule->id) }}" method="POST" class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div>
                            ...
                            <textarea name="content" id="content" rows="4"
                                class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                                placeholder="Escribe el contenido que se publicará..." required>{{ old('content', $schedule->content) }}</textarea>
                                    ...
                                    <div class="flex items-center">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-sm text-amber-700">Conecta al menos una cuenta social en <a
                                                    href="{{ route('settings') }}"
                                                    class="underline hover:no-underline">Configuración</a> para usar
                                ...
                                @forelse($availablePlatforms as $platform)
                                    ...
                                    <label
                                        ...
                                        <input type="checkbox" name="platforms[]" value="{{ $platform['value'] }}"
                                            ...
                                            {{ in_array($platform['value'], old('platforms', $schedule->platforms ?? [])) ? 'checked' : '' }}
                            ...
                            <div class="space-y-4">
                                <label class="flex items-start">
                                    <input type="radio" name="is_recurring" value="0"
                                        ...
                                        {{ old('is_recurring', $schedule->is_recurring ? '1' : '0') == '0' ? 'checked' : '' }}>
                                    <div class="ml-3">
                                ...
                                <label class="flex items-start">
                                    <input type="radio" name="is_recurring" value="1"
                                        class="mt-1 h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300"
                                        {{ old('is_recurring', $schedule->is_recurring ? '1' : '0') == '1' ? 'checked' : '' }}>
                                    <div class="ml-3">
                        ...
                        <div id="specific-schedule"
                            class="space-y-4 {{ old('is_recurring', $schedule->is_recurring ? '1' : '0') == '1' ? 'hidden' : '' }}">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="date" name="schedule_date" id="schedule_date"
                                        value="{{ old('schedule_date', $schedule->schedule_date ? $schedule->schedule_date->format('Y-m-d') : '') }}"
                                        class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                                    @error('schedule_date')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                                ...
                                <div>
                                    ...
                                    <input type="time" name="schedule_time" id="schedule_time"
                                        value="{{ old('schedule_time', $schedule->schedule_time ? $schedule->schedule_time->format('H:i') : '09:00') }}"
                                        class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                                    @error('schedule_time')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                        ...
                        <div id="recurring-schedule"
                            ...
                            <div>
                                ...
                                <div class="grid grid-cols-4 md:grid-cols-7 gap-2">
                                    ...
                                    @foreach ($days as $index => $day)
                                        <label
                                            class="flex flex-col items-center p-2 border border-gray-300 rounded-lg cursor-pointer hover:bg-gray-50">
                                            <input type="checkbox" name="recurring_days[]"
                                                value="{{ $index }}"
                                                class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded mb-1"
                                                {{ in_array($index, $recurringDays) ? 'checked' : '' }}>
                                            <span
                            ...
                            <div>
                                ...
                                <input type="time" name="recurring_time" id="recurring_time"
                                    value="{{ old('recurring_time', $schedule->recurring_time ? $schedule->recurring_time->format('H:i') : '09:00') }}"
                                    class="w-full md:w-48 px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="date" name="recurring_start_date" id="recurring_start_date"
                                        value="{{ old('recurring_start_date', $schedule->recurring_start_date ? $schedule->recurring_start_date->format('Y-m-d') : '') }}"
                                        class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                                </div>
                        ...
                        <div>
                            ...
                            <textarea name="notes" id="notes" rows="3"
                                class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                                placeholder="Notas adicionales sobre este horario...">{{ old('notes', $schedule->notes) }}</textarea>
                        </div>
                        ...
                        <div>
                            ...
                            <select name="status" id="status"
                                class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                                <option value="active"
                                    {{ old('status', $schedule->status) == 'active' ? 'selected' : '' }}>Activo
                                </option>
                                <option value="cancelled"
                                    {{ old('status', $schedule->status) == 'cancelled' ? 'selected' : '' }}>Cancelado
                                </option>
                            </select>
                        </div>
